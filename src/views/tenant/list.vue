<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.info" placeholder="tenant_id/租户名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-button v-waves class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">
        搜索
      </el-button>
      <router-link :to="'/tenant/tenant_create'">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;">
      ...
      <el-table-column label="tenant_id" min-width="50px">
        <template slot-scope="{row}">
      ...
      <el-table-column label="操作" align="center" min-width="90" class-name="small-padding fixed-width">
        <template slot-scope="{ row }">
          <router-link :to="'/tenant/tenant_statistics/' + row.id">
            <el-button type="primary" size="mini">
              统计
            </el-button>
          </router-link>
          <router-link :to="'/tenant/tenant_edit/' + row.id">
            <el-button type="primary" size="mini">
              修改
            </el-button>
          </router-link>
          <el-button type="mini" size="danger" @click="handleDelete(row)">
            删除
          </el-button>
        </template>
      </el-table-column>
...
<script>
import { tenantList, tenantDelete } from '@/api/tenant'
...
export default {
  ...
  methods: {
    getList() {
      ...
      tenantList(this.listQuery).then(response => {
